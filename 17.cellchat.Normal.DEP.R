...
out_dir <- file.path(base_dir, 'cellchat_Normal_DEPR')
...
# Filter for Normal (cognitively normal) subjects only
...
print('Number of subjects with Normal cognition but without depression:')
print(nrow(clinic[clinic$diagnosis == 'Normal_Normal', ]))
print('Number of subjects with Normal cognition and depression:')
...
cell.use1 <- rownames(meta)[meta$diagnosis == "Normal_DEPR"] # Normal cognition with depression
meta1 <- meta[cell.use1, ]
print(paste("Normal_DEPR cells:", length(cell.use1)))
print("Normal_DEPR cell types:")
print(table(meta1$mayo.cell.type))

cell.use2 <- rownames(meta)[meta$diagnosis == "Normal_Normal"] # Normal cognition without depression  
meta2 <- meta[cell.use2, ]
print(paste("Normal_Normal cells:", length(cell.use2)))
print("Normal_Normal cell types:")
...
print("Common cell types between Normal_DEPR and Normal_Normal:")
print(cell.type)
...
if(length(cell.type) > 0) {
  cell_type_counts <- data.frame(
    CellType = cell.type,
    Normal_DEPR = sapply(cell.type, function(x) sum(meta1$mayo.cell.type == x, na.rm = TRUE)),
    Normal_Normal = sapply(cell.type, function(x) sum(meta2$mayo.cell.type == x, na.rm = TRUE))
  )
  ...
  sufficient_cell_types <- cell_type_counts$CellType[
    cell_type_counts$Normal_DEPR >= 10 & cell_type_counts$Normal_Normal >= 10
  ]
...
###################################################################
# Analysis for Normal_DEPR (Normal cognition with depression)
###################################################################

# Extract cells for Normal_DEPR condition with sufficient cell types
cell.use1 <- rownames(meta)[meta$diagnosis == "Normal_DEPR" & meta$mayo.cell.type %in% cell.type]
data.input1 <- data.input[, cell.use1]
metadata1 <- data.frame(labels = meta[cell.use1, "mayo.cell.type"], row.names = cell.use1)

print(paste("Normal_DEPR cells for CellChat:", length(cell.use1)))
print("Normal_DEPR cell type distribution for CellChat:")
print(table(metadata1$labels))

# Create CellChat object for Normal_DEPR
...
# Save results
setwd(out_dir)
saveRDS(cellchat1, 'Normal_DEPR.cellchat.rds')

###################################################################
# Analysis for Normal_Normal (Normal cognition without depression)
###################################################################

# Extract cells for Normal_Normal condition with sufficient cell types
cell.use2 <- rownames(meta)[meta$diagnosis == "Normal_Normal" & meta$mayo.cell.type %in% cell.type]
data.input2 <- data.input[, cell.use2]
metadata2 <- data.frame(labels = meta[cell.use2, "mayo.cell.type"], row.names = cell.use2)

print(paste("Normal_Normal cells for CellChat:", length(cell.use2)))
print("Normal_Normal cell type distribution for CellChat:")
print(table(metadata2$labels))

# Create CellChat object for Normal_Normal
...
# Save results
setwd(out_dir)
saveRDS(cellchat2, 'Normal_Normal.cellchat.rds')

###################################################################
# Comparative analysis between Normal_DEPR and Normal_Normal
###################################################################

object.list <- list(Normal_DEPR=cellchat1, Normal_Normal=cellchat2)
cellchat <- mergeCellChat(object.list, add.names = names(object.list))
cellchat

# Visualize differential number of interactions
png(filename = 'Normal_DEPR_vs_Normal.cc.diff.number.inter.network.png', width = 2000, height = 2000, res = 300)
netVisual_diffInteraction(cellchat, weight.scale = T, comparison = c("Normal_DEPR","Normal_Normal"))
dev.off()

# Networks: red (or blue) colored edges represent increased (or decreased) signaling in Normal_DEPR compared to Normal_Normal

# Visualize differential interaction strength
png(filename = 'Normal_DEPR_vs_Normal.cc.diff.inter.strength.network.png', width = 2000, height = 2000, res = 300)
netVisual_diffInteraction(cellchat, weight.scale = T, measure = "weight", comparison = c("Normal_DEPR","Normal_Normal"))
dev.off()

# Heatmap visualization
# In the colorbar, red (or blue) represents increased (or decreased) signaling in Normal_DEPR compared to Normal_Normal
gg1 <- netVisual_heatmap(cellchat, comparison = c(1,2))
gg2 <- netVisual_heatmap(cellchat, measure = "weight", comparison = c(1,2))

png(filename = 'Normal_DEPR_vs_Normal.cc.diff.number.inter.heatmap.png', width = 2000, height = 2000, res = 300)
print(gg1)
dev.off()

png(filename = 'Normal_DEPR_vs_Normal.cc.diff.inter.strength.heatmap.png', width = 2000, height = 2000, res = 300)
print(gg2)
dev.off()

# Save merged CellChat object
saveRDS(cellchat, 'Normal_DEPR_vs_Normal.cellchat.rds')

# Compare overall interaction numbers and strengths
gg1 <- compareInteractions(cellchat, show.legend = F, group = c("Normal_DEPR","Normal_Normal"))
gg2 <- compareInteractions(cellchat, show.legend = F, group = c("Normal_DEPR","Normal_Normal"), measure = "weight")

png(filename = 'Normal_DEPR_vs_Normal.cc.inter.number.bar.png', width = 1000, height = 2000, res = 300)
print(gg1)
dev.off()

png(filename = 'Normal_DEPR_vs_Normal.cc.inter.weight.bar.png', width = 1000, height = 2000, res = 300)
print(gg2)
dev.off()
...
png(filename = 'Normal_DEPR_vs_Normal.cc.signal.pathways.relative.info.flow.heatmap.png', width = 1000, height = 5000, res = 200)
print(gg1)
dev.off()

png(filename = 'Normal_DEPR_vs_Normal.cc.signal.pathways.info.flow.heatmap.png', width = 1000, height = 5000, res = 200)
print(gg2)
dev.off()
...
png(filename = 'Normal_DEPR.signaling.role.scatter.png', width = 2000, height = 2000, res = 200)
print(gg[[1]])
dev.off()

png(filename = 'Normal_Normal.signaling.role.scatter.png', width = 2000, height = 2000, res = 200)
print(gg[[2]])
dev.off()
...
writeLines(c(
  paste("CellChat Analysis Summary - Normal Cognition Depression Study"),
  paste("Date:", Sys.Date()),
  paste("Comparison: Normal_DEPR vs Normal_Normal"),
  ...
  paste("- Normal_DEPR cells for analysis:", length(cell.use1)),
  paste("- Normal_Normal cells for analysis:", length(cell.use2)),
  paste(""),
  ...
  paste("- Normal_DEPR.cellchat.rds"),
  paste("- Normal_Normal.cellchat.rds"), 
  paste("- Normal_DEPR_vs_Normal.cellchat.rds"),
