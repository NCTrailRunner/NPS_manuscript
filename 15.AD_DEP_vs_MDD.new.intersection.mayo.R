...
out.dir <- '/hpc/group/adrc/zm77/LUTZ.MDD/ROSMAP.BIG.10032024/new.analysis.03112025/objects/summary/AD_DEP_ovl_MDD'
...
setwd ('/hpc/group/adrc/zm77/LUTZ.MDD/MDD_new/objects/nebula_results')
degs_list <- list.files(pattern = "_two_step_FDR.csv")
...
degs <- lapply (1:length (degs.all),
                function (x) {
                tmp <- degs.all[x]
				cluster <- gsub ('MDD_vs_Control_|_filtered_two_step_FDR.csv', '', names(tmp))
				tmp <- as.data.frame (degs.all[[x]])
				tmp$cell.type <- cluster
				rownames (tmp) <- NULL
				tmp <- tmp[order(tmp$FDR_Adj_p_val), ]
...
setwd (out.dir)
write.csv (degs.mdd, 'MDD.NEBULA.DEGs.two_step.csv')
...
# =============================================================================
message("Analyzing all DEGs (up and down combined) between AD and MDD...")
...
for (j in (1: length(cell.type)))
{
  ...
  # Get ALL DEGs (both up and down) for MDD in this cell type
  ...
  print(paste("MDD DEGs:", length(set2.all)))
  ...
  names(uniq.all[[j]]) <- list('AD_DEP', 'MDD')
  
  # Create Venn diagram for all DEGs
  setwd(out.dir)
  inter_list_all <- list('AD_with_DEP' = set1.all, 'MDD' = set2.all)
  venn.all <- ggvenn(inter_list_all, fill_color = c("purple", "blue"))
  
  # Save Venn diagram
  png(filename = paste0('AD_DEP_vs_MDD_', cell.type[j], '_', 'degs.venn.two_step.png'))
  print(venn.all)
  dev.off()
}
...
setwd(out.dir)
file.all <- "AD_DEP_vs_MDD.celltype.common.ALL.two_step.txt"
...
output_file_all <- "LOAD_DEP_MDD.celltype.unique.ALL.two_step.txt"
...
summary_all <- data.frame(
  ...
  MDD_DEGs = sapply(names(inter.all), function(ct) length(degs.mdd[degs.mdd$cell.type == ct, ]$gene)),
  Common_DEGs = sapply(inter.all, length),
  Unique_AD = sapply(uniq.all, function(x) length(x$AD)),
  Unique_MDD = sapply(uniq.all, function(x) length(x$MDD))
)

# Calculate percentages
summary_all$Percent_Common_of_AD <- round((summary_all$Common_DEGs / summary_all$AD_DEGs) * 100, 2)
summary_all$Percent_Common_of_MDD <- round((summary_all$Common_DEGs / summary_all$MDD_DEGs) * 100, 2)

# Save summary table
write.csv(summary_all, "LOAD.MDD.DEGs.summary.ALL.mayo.csv", row.names = FALSE)
...
for (j in (1: length(cell.type)))
{
  ...
  names (uniq[[j]]) <- list ('AD_DEP', 'MDD')
  setwd (out.dir)
  inter_list <-list('AD_DEP'=set1.up,'MDD'=set2.up)
  venn <- ggvenn(inter_list,  fill_color=c("purple", "blue"))
  png(filename = paste0('AD_DEP_vs_MDD_', cell.type[j], '_', 'degs.venn.celltype.up.two_step.png'))
  print(venn)
...
# setwd (mdd.dir)
# data2 <- read.csv (paste0 ('MDD.CONTROL.', cell.type[3], '_sig_genes_NEBULA.celltype.mayo.csv'))
...
setwd (out.dir)
file <- "AD_DEP_vs_MDD.celltype.common.up.two_step.txt"
...
output_file <- "AD_DEP_vs_MDD.celltype.unique.up.two_step.txt"
...
write_list <- function(data_list, file_connection) {
  ...
  for (top_level in names(data_list)) {
    ...
    # Get sub-elements (e.g., $Astrocyte$AD, $Astrocyte$MDD)
    sub_elements <- data_list[[top_level]]
...
for (j in (1: length(cell.type)))
{
  ...
  names (uniq[[j]]) <- list ('AD_DEP', 'MDD')
  setwd (out.dir)
  inter_list <-list('AD_DEP'=set1.down,'MDD'=set2.down)
  venn <- ggvenn(inter_list,  fill_color=c("purple", "blue"))
  png(filename = paste0('AD_DEP_vs_MDD_', cell.type[j], '_', 'degs.venn.celltype.down.two_step.png'))
  print(venn)
  #dev.copy(png,paste0('three_path_', cell.type[j], '_', 'degs.venn.png'))
  dev.off()
...
setwd (out.dir)
file <- "AD_DEP_vs_MDD.celltype.common.down.two_step.txt"
conn <- file(description=file, open="w")
...
output_file <- "AD_DEP_vs_MDD.celltype.unique.down.two_step.txt"
